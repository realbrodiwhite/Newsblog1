...
// src/version.supported.ts
var version_supported_default = [
  {
    flutter: "3.10.4",
    dart: "3.0.3",
    flutterNumber: 1004,
...
// src/version.ts
var SupportedVersions = version_supported_default.map(
  (x) => __spreadValues({}, x)
);
var FallbackStableVersion = __spreadValues({}, SupportedVersions.find((x) => x.channel === "stable"));
var FallbackBetaVersion = __spreadValues({}, SupportedVersions.find((x) => x.channel === "beta"));
...
function versionPartToNumber(part, index) {
  const base = __pow(10, 2 - index) * Number.parseInt(part) * (index === 0 && part > "1" ? (index + 1) * 3 : 1);
  if (part.length === 1) {
    return base;
  }
...
// src/matcher.ts
function matcher_default(dart, channel = "stable") {
  const version = dart;
  ...
  return higherVersions.length > 0 ? higherVersions[higherVersions.length - 1] : channel === "stable" ? FallbackStableVersion : FallbackBetaVersion;
}

// src/parser.ts
var lockDartVersionRegex = /dart: ["|'](.){0,2}(([0-9]{1,}\.*){3})(.*)["|']/g;
var lockFlutterVersionRegex = /flutter: ["|'](.){0,2}(([0-9]{1,}\.*){3})(.*)["|']/g;
...
function parser_default(yaml) {
  ...
  const dartVersion = (_a = extractDartVersionFromLock(yaml)) != null ? _a : extractDartVersionFromYAML(yaml);
  return {
...
function extractFlutterVersionFromLock(yaml) {
  var _a;
  lockFlutterVersionRegex.lastIndex = 0;
  return (_a = lockFlutterVersionRegex.exec(yaml)) == null ? void 0 : _a.at(2);
}
...
// src/handler.ts
function handler_default(yaml, channel) {
  var _a;
  const versions = parser_default(yaml);
  const parsedFlutterVersion = versions.flutter;
  const parsedDartVersion = (_a = versions.dart) != null ? _a : UnknownDartVersion;
  const matchedVersion = matcher_default(parsedDartVersion, channel);
  ...
  return __spreadProps(__spreadValues({}, matchedVersion), {
